...
非常感谢 [laradock](https://github.com/LaraDock/laradock)， [phpdocker](https://github.com/Joker779/phpdocker) 是简化定制之后的产物，方便学习使用。
phpdocker
...
```bash
git clone https://github.com/Joker779/phpdocker
cd phpdocker
cp .env.example .env
```
...
phpdocker 默认将同级目录下的所有文件映射到数据卷容器 applications 中。其实可以你完全可以灵活配置，添加多个映射，例如：
...
`docker-compose exec -it -u phpdocker workspace bash`

增加 --user=phpdocker (例如 docker-compose exec --user=phpdocker workspace bash) 作为您的主机的用户创建的文件. (你可以从 docker-compose.yml 修改 PUID (User id) 和 PGID (group id) 值 )。
...
* 你也可以使用以下命令查看当前 phpdocker 启动的容器
...
```php
\Cache::store('redis')->put('phpdocker', 'Awesome', 10);
```
